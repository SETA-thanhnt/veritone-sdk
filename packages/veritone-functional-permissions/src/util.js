export function combinePermissions(permissions1, permissions2) {
  ...
  const maxIndex =
    permissions1.length > permissions2.length
      ? permissions1.length
      : permissions2.length;
...
export function getPermissionIdsFromMask(permissions) {
...
export function getMaskFromPermissionIds(permissionIds, mask) {
...
export function hasAccessTo(permissionId, userPermissions) {
  if (
...
export function hasAccessToAny(permissionIds, userPermissions) {
...
export function hasAccessToAll(permissionIds, userPermissions) {
  if (!Array.isArray(permissionIds) || !Array.isArray(userPermissions)) {
    return false;
  }
